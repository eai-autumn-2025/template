...
def error(text: str) -> int:
    ...
    standart_fraquency = {        ' ': 17.1662,
        'e': 10.2911, 't': 7.9483, 'a': 7.4825, 'o': 7.2201,      'n': 6.7703, 'i': 6.3456, 's': 6.1473, 'r': 5.9951,        'h': 5.2448, 'l': 4.1253, 'd': 3.7879, 'c': 3.3854,        'u': 2.7584, 'm': 2.7055, 'f': 2.4063, 'p': 2.1885,
        'g': 2.0306, 'w': 1.9541, 'y': 1.8510, 'b': 1.8194,        'v': 1.0077, 'k': 0.9678, 'x': 0.1923, 'j': 0.1533,
        'q': 0.0957, 'z': 0.0776,        'E': 0.6492, 'T': 0.5976, 'A': 0.5801, 'O': 0.5043,
        ...
        'Q': 0.0252, 'Z': 0.0201,
        ',': 1.2345, '.': 1.1201, '"': 0.8654, "'": 0.7543,       '?': 0.4521, '!': 0.3521, ':': 0.2854, ';': 0.2154,       '-': 0.1854, '(': 0.1452, ')': 0.1452, '[': 0.0452,       ']': 0.0452, '{': 0.0252, '}': 0.0252, '<': 0.0152,
    ...
    letters = sorted(list(letters),key = lambda x: -standart_fraquency[x])
    return  sum(abs(fraquency[letters[i]] - standart_fraquency[letters[i]]) * (i+8)   for i in range(len(letters)))
...
def hack(ciphertext: str) -> tuple[str, int]:
    ...
    for shift in range(interval_end - interval_begin + 1):
        ...
        res.append((int(decrypted_error),caesar.decrypt(ciphertext, shift)))
